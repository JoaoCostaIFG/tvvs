...
projects that want to enforce a coding standard. It can find both class and
methods design problems, and it has also the ability to check code layout and
formatting issues.
...
. The rules were adapted to reduce the amount of irrelevant issues. These
...
- **Underscore in root package name** - There were multiple warnings related to
  the root package name, because it contains an underscore, `_`. In the default
...
- **Import declaration groups** - The tool was indicating that there was an
  ...
  author of the project organized the import declarations by grouping third
  party packages separately from the java standard ones, we adapted this rule to
...
as indentations. After the indentation settings were fixed, most warnings are
related to mixing spaces and tabs for indentation.
...
expected level should be 16_. **Location**: file `JTimeSchedApp.java`, lines
100, 389, and 401.
...
level should be 16._ **Location**: file `JTimeSchedFrame.java`, lines 136, 709,
712, 714, and 715 (and other locations).
...
**Warning**: _'{' at column 3 should be on the previous line_. **Location**:
file `ProjectSerializer.java` line 81.
...
should be one of the following: 24, 32._ **Location**: file
`ProjectSerializer.java` lines 82, 84, 85, and 86.
...
48_. **Location**: file `CustomCellRenderer.java` lines 71-76.
...
There are numerous warnings related to weird instantiation of Integers, Longs,
...
de.dominik_geyer.jtimesched.JTimeSchedApp.main(String[])_. **Location**: file
`JTimeSchedApp.java` line 57.
...
instead_. **Location**: file `ProjectSerializer.java` line 66.
...
**Warning**: _inefficient Boolean constructor; use Boolean.valueOf(...)
instead_. **Location**: file `ProjectTableModel.java` lines 75 and 81.
...
**Warning**: _inefficient new Integer(int) constructor; use Integer.valueOf(int)
instead_. **Location**: file `ProjectTableModel.java` lines 84 and 87.
...
**Warning**: _default case is missing_. **Location**: file
`JTimeSchedFrame.java` line 971.
